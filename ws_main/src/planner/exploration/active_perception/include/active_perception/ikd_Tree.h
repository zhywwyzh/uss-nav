...
struct ikdTree_PolyhedronType{
    ...
    ikdTree_PolyhedronType (PolyHedronPtr poly_ptr = nullptr){
      if (poly_ptr != nullptr){
        ...
        x = static_cast<float>(poly_ptr->origin_center_.x());
        y = static_cast<float>(poly_ptr->origin_center_.y());
        z = static_cast<float>(poly_ptr->origin_center_.z());
      }else{
        // std::cout << "[ikdTree] | ERROR | polyhedron_ is nullptr! skip this polyhedron..." << std::endl;
        x = y = z = 0.0f;
      }
    }
    void set_coordinate(const Eigen::Vector3d pt){
      x = static_cast<float>(pt.x());
      y = static_cast<float>(pt.y());
      z = static_cast<float>(pt.z());
    }
};*/

struct ikdTree_VPType {
    float x,y,z;
    ego_planner::Viewpoint::Ptr vp_{nullptr};
    ikdTree_VPType (ego_planner::Viewpoint::Ptr vp_ptr = nullptr){
        if (vp_ptr != nullptr) {
            vp_ = vp_ptr;
            x = static_cast<float>(vp_ptr->pos_.x());
            y = static_cast<float>(vp_ptr->pos_.y());
            z = static_cast<float>(vp_ptr->pos_.z());
        }else {
            // std::cout << "[ikdTree] | ERROR | vp_ is nullptr! skip this vp..." << std::endl;
            x = y = z = 0.0f;
        }
    };
    ikdTree_VPType(const Eigen::Vector3d pt) {
        x = static_cast<float>(pt.x());
        y = static_cast<float>(pt.y());
        z = static_cast<float>(pt.z());
        vp_ = nullptr;
...
struct ikdTree_Vectoe3dType{
  float x,y,z;
  Eigen::Vector3d vec3d;
  ikdTree_Vectoe3dType (Eigen::Vector3d pt = Eigen::Vector3d::Zero()){
    x = static_cast<float>(pt.x());
    y = static_cast<float>(pt.y());
    z = static_cast<float>(pt.z());
    vec3d = pt;
