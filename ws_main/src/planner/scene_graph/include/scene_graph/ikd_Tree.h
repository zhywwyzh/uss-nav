...
struct ikdTree_ObjectDataType{
  ...
  ikdTree_ObjectDataType (ObjectNode::Ptr obj_ptr = nullptr){
    if (obj_ptr != nullptr){
      obj_ = obj_ptr;
      x = static_cast<float>(obj_ptr->pos.x());
      y = static_cast<float>(obj_ptr->pos.y());
      z = static_cast<float>(obj_ptr->pos.z());
    }else{
      x = y = z = 0.0f;
    }
  }
...
struct ikdTree_PolyhedronType_FixedCenter{
  ...
  ikdTree_PolyhedronType_FixedCenter (PolyHedronPtr poly_ptr = nullptr){
    if (poly_ptr != nullptr){
      ...
      x = static_cast<float>(poly_ptr->center_.x());
      y = static_cast<float>(poly_ptr->center_.y());
      z = static_cast<float>(poly_ptr->center_.z());
    }else{
