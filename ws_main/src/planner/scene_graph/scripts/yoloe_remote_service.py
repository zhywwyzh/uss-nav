...
# -*- coding: utf-8 -*-
"""
yoloe_remote_service.py
=======================
YOLOE 远程推理服务（纯 Python、非 ROS 外部进程，机载运行）。

与基于 ROS 的 yoloe_server.py 功能对等，但把 ROS 话题通信替换为 TCP 通信：
- 本脚本作为 TCP server 监听 127.0.0.1:9010，接收广播器（C++ ROS 节点）
  通过 TCP 发来的 rgb / depth / odom 三路帧数据；
- 内部完成与 yoloe_server.py 一致的 YOLOE 分割 + MobileCLIP 文本编码推理；
- 推理结果（含原始 rgb/depth 压缩字节与各检测目标的 mask PNG / 512 维
  word_vector）通过同一 TCP 连接以 JSON 形式回发给广播器，由广播器负责
  重建 EncodeMask 消息。

本脚本不依赖 rospy / cv_bridge，仅依赖 opencv / numpy / torch /
ultralytics(YOLOE) / mobileclip / base64 / json / struct / socket / threading。

帧协议（与广播器 C++ 端共享，全部大端序 / network order）：
...
class YoloeRemoteService:
    """YOLOE 远程推理服务。

    作为 TCP server 接收广播器上行帧（rgb/depth/odom），rgb 作为触发帧，
    三路 stamp 同步后启动推理线程，结果通过下行帧（topic_id=100, JSON）
    回发给广播器。
    """
    ...
    def __init__(self, host="127.0.0.1", port=9010,
                 ...
                 conf=0.4, time_slop=0.05, device="cuda:0"):
        # ---- 网络与推理参数 ----
        self.host = host
        self.port = port
        ...
        self.device = device
        self.resize_width = 640             # 推理前将 RGB 宽度缩放到 640（与原实现一致）

        # ---- 模型路径与开关（默认值与原 yoloe_server.py 一致） ----
        ...
        try:
            ...
            print(f"[YoloeRemoteService] 模型加载失败: {e}")
            sys.exit(1)

    # ------------------------------------------------------------------ #
    # 模型加载（逻辑照搬 yoloe_server.py 的 __init__）
    # ------------------------------------------------------------------ #
    def _load_models(self):
        if not torch.cuda.is_available():
            print("[YoloeRemoteService] CUDA 不可用，无法运行 YOLOE 推理")
            sys.exit(1)

        # Prompt 相关初始化：先用未融合模型根据 prompt 文本计算 vocab
        if self.use_prompt:
            print("[YoloeRemoteService] 已启用 Prompt 模式，加载 prompt 模型并计算 vocab")
            ...
            print(f"[YoloeRemoteService] vocab 计算完成，共 {len(self.names)} 个类别")
        ...
        print("[YoloeRemoteService] 模型初始化完成")
    ...
    def start(self):
        ...
        server.listen(1)
        print(f"[YoloeRemoteService] 开始监听 {self.host}:{self.port} (time_slop={self.time_slop}s)")
        try:
            while True:
                conn, addr = server.accept()
                print(f"[YoloeRemoteService] 广播器已连接: {addr}")
                ...
                print(f"[YoloeRemoteService] 广播器连接断开: {addr}")
        except KeyboardInterrupt:
            # 仅处理 Ctrl-C 优雅退出，不吞掉其他异常
            print("[YoloeRemoteService] 收到 Ctrl-C，服务退出")
        finally:
            server.close()
    ...
    def _handle_connection(self, conn):
        ...
        while True:
            try:
                ...
                print(f"[YoloeRemoteService] recv 出错，连接终止: {e}")
                break
            if not data:
                # 对端关闭连接
                print("[YoloeRemoteService] 对端关闭连接")
                break
    ...
    def _dispatch(self, frame, conn):
        ...
        if topic_id == TOPIC_RGB:
            ...
            print(f"[YoloeRemoteService] 未知 topic_id={topic_id}，丢弃该帧")
    ...
    def _try_infer(self, conn):
        ...
        if abs(depth_stamp - rgb_stamp) > self.time_slop or abs(odom_stamp - rgb_stamp) > self.time_slop:
            print(f"[YoloeRemoteService][debug] 等待同步: rgb={rgb_stamp:.3f} "
                  f"depth={depth_stamp:.3f} odom={odom_stamp:.3f}")
            return

        # 同一时刻只允许一个推理线程：上一帧推理未结束时跳过当前触发帧
        if not self._infer_lock.acquire(blocking=False):
            print("[YoloeRemoteService][debug] 上一帧推理仍在进行，跳过当前触发帧")
            return

        rgb = self._latest_rgb
        depth = self._latest_depth
        odom = self._latest_odom
        # 异步推理：避免阻塞收帧循环（参考 vlm_detect_server.py 的线程模式）
    ...
    def _run_infer(self, conn, rgb, depth, odom):
        ...
        try:
            ...
            print(f"[YoloeRemoteService] 推理线程异常: {e}")
        finally:
            self._infer_lock.release()

    def _infer(self, rgb_bgr, depth, odom, stamp, rgb_bytes, depth_bytes):
        """核心推理（逻辑照搬 yoloe_server.py processing_loop 的推理部分）。
        ...
        """
        # 1. BGR -> RGB（与原实现一致），并缩放至宽度 640 保持宽高比
            ...
            for i in range(len(mask_arrays)):
                ...
                try:
                    ...
                    if ok:
                        ...
                        print(f"[YoloeRemoteService] 掩码 {i} PNG 编码失败")
                except Exception as e:
                    # 个别掩码转换失败不应中断整帧处理
                    print(f"[YoloeRemoteService] 掩码 {i} 编码异常: {e}")

        # 5. MobileCLIP 文本编码：tokenizer -> encode_text -> 归一化 -> float64[512]
        if self.use_clip:
            try:
                text = self.clip_tokenizer(labels).to("cuda:0")
                with torch.no_grad():
    ...
    def _send_result(self, conn, result_dict):
        ...
        with self._send_lock:
            try:
                ...
                # socket 断开是必须处理的异常边界
                print(f"[YoloeRemoteService] 发送结果失败: {e}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="YOLOE 远程推理服务（纯 Python TCP server，非 ROS）")
    ...
    parser.add_argument("--time_slop", type=float, default=0.05,
                        help="三路帧时间同步容差，秒（默认 0.05）")
    args = parser.parse_args()

    service = YoloeRemoteService(
        host=args.host,
        port=args.port,
