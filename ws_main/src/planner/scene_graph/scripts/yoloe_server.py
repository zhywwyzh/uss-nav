...
try:
    from std_msgs.msg import EncodeMask
    from std_msgs.msg import WordVector
except ImportError:
...
class YoloeDetectorNode:
    def __init__(self):
        ...
        # 三个输入话题参数
        self.rgb_in_topic        = rospy.get_param('~rgb_topic', '/camera/color/image/compressed')
        self.depth_in_topic      = rospy.get_param('~depth_topic', '/camera/depth/image/compressed')
        self.odom_in_topic       = rospy.get_param('~odom_topic', '/unity_depth_odom')
