...
float KD_TREE<PointType>::calc_box_dist(KD_TREE_NODE *node, PointType point) {
  ...
  float min_dist = 0.0;
  if (point.x < node->node_range_x[0]) min_dist += (point.x - node->node_range_x[0]) *
                                                   (point.x - node->node_range_x[0]);
  if (point.x > node->node_range_x[1]) min_dist += (point.x - node->node_range_x[1]) *
                                                   (point.x - node->node_range_x[1]);
  if (point.y < node->node_range_y[0]) min_dist += (point.y - node->node_range_y[0]) *
                                                   (point.y - node->node_range_y[0]);
  if (point.y > node->node_range_y[1]) min_dist += (point.y - node->node_range_y[1]) *
                                                   (point.y - node->node_range_y[1]);
  if (point.z < node->node_range_z[0]) min_dist += (point.z - node->node_range_z[0]) *
                                                   (point.z - node->node_range_z[0]);
  if (point.z > node->node_range_z[1]) min_dist += (point.z - node->node_range_z[1]) *
...
template
class KD_TREE<ikdTree_ObjectDataType>;

template
class KD_TREE<ikdTree_PolyhedronType>;

template
class KD_TREE<ikdTree_PolyhedronType_FixedCenter>;

template
class KD_TREE<ikdTree_Vectoe3dType>;
