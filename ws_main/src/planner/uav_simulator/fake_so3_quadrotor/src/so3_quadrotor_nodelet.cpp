...
class Nodelet : public nodelet::Nodelet {
 private:
  ...
  tf::TransformBroadcaster tf_view_br_;
  ...
  void timer_callback(const ros::TimerEvent& event) {
    ...
    if (tnow >= next_odom_pub_time) {
      ...
      if(is_target){
        tf_br_.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "world", "body_target"));
      }else{
        tf_br_.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "world", "body"));
      }
      // tf_view
      if(is_target){
        tf::Transform transform_view;
        transform_view.setOrigin( tf::Vector3(pos.x(), pos.y(), pos.z()) );
        transform_view.setRotation( tf::Quaternion(0, 0, 0, 1) );
        tf_view_br_.sendTransform(tf::StampedTransform(transform_view, ros::Time::now(), "world", "view"));
      }
      // odom
