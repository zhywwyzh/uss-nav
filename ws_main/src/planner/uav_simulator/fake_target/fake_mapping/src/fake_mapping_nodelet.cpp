...
class Nodelet : public nodelet::Nodelet {
 private:
  ...
  message_filters::Subscriber<sensor_msgs::PointCloud2> local_pcl_sub_;
  ...
  void depth_odom_callback(const sensor_msgs::ImageConstPtr& depth_msg,
    ...
    quadrotor_msgs::OccMap3dOld gridmap_msg;
  ...
  {
    cam_p_(0) = odom->pose.pose.position.x;
    cam_p_(1) = odom->pose.pose.position.y;
    cam_p_(2) = odom->pose.pose.position.z;
  ...
  void cloudCallback(const sensor_msgs::PointCloud2ConstPtr &img)
  {
    ...
    if (isnan(cam_p_(0)) || isnan(cam_p_(1)) || isnan(cam_p_(2)))
      return;
    
    if (latest_cloud.points.size() > 0){
      Eigen::Vector3d body_p = cam_p_;
      std::vector<Eigen::Vector3d> obs_pts;
      for(int i = 0; i < latest_cloud.size(); i++){
        pcl::PointXYZ p = latest_cloud.points[i];
        obs_pts.emplace_back(p.x, p.y, p.z);
      }
      gridmap_.updateMap(body_p, obs_pts);
    }
    ...
    gridmap_.to_msg(gridmap_msg);
    gridmap_inflate_pub_.publish(gridmap_msg);
    // ROS_WARN_STREAM("publish gridmap_inflate_pub_");
  }

  // void pcl_odom_callback(const sensor_msgs::PointCloud2ConstPtr& pcl_msg,
  //                         const nav_msgs::OdometryConstPtr& odom_msg) {

  //   ROS_WARN_STREAM("pcl_odom_callback");
  //   if (callback_lock_.test_and_set()) {
  //     return;
  //   }
  //   Eigen::Vector3d body_p(odom_msg->pose.pose.position.x,
  ...
  //                       odom_msg->pose.pose.position.z);
  //   std::vector<Eigen::Vector3d> obs_pts;
  //   pcl::PointCloud<pcl::PointXYZ> local_pc;
  //   pcl::fromROSMsg(*pcl_msg, local_pc);
  //   for(int i = 0; i < local_pc.size(); i++){
  //     pcl::PointXYZ p = local_pc.points[i];
  //     obs_pts.emplace_back(p.x, p.y, p.z);
  //   }
  //   gridmap_.updateMap(body_p, obs_pts);
  ...
  void init(ros::NodeHandle& nh) {
    ...
    nh.getParam("use_depth", use_depth_);
    ...
    gridmap_inflate_pub_ = nh.advertise<quadrotor_msgs::OccMap3dOld>("gridmap_inflate", 1);
    ...
    if (use_global_map_) {
      ...
      if(use_depth_){
        ...
        ROS_WARN_STREAM("do not use depth");
        indep_cloud_sub_ =
            nh.subscribe<sensor_msgs::PointCloud2>("local_cloud", 10, &Nodelet::cloudCallback, this);
        indep_odom_sub_ =
            nh.subscribe<nav_msgs::Odometry>("odom", 10, &Nodelet::odomCallback, this);


        // local_pcl_sub_.subscribe(nh, "local_cloud", 1);
        // pcl_odom_sync_Ptr_ = std::make_shared<PclOdomSynchronizer>(PclOdomSyncPolicy(10), local_pcl_sub_, odom_sub_);
        // pcl_odom_sync_Ptr_->registerCallback(boost::bind(&Nodelet::pcl_odom_callback, this, _1, _2));
      }
...
PLUGINLIB_EXPORT_CLASS(fake_mapping::Nodelet, nodelet::Nodelet);