...
void gridmap_callback(const quadrotor_msgs::OccMap3dOldConstPtr& msgPtr) {
  fake_mapping::OccGridMap gridmap;
...
void gridmap_inflate_callback(const quadrotor_msgs::OccMap3dOldConstPtr& msgPtr) {
  fake_mapping::OccGridMap gridmap;
  gridmap.from_msg(*msgPtr);
  sensor_msgs::PointCloud2 pc;
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber gridmap_sub = nh.subscribe<quadrotor_msgs::OccMap3dOld>("gridmap", 1, gridmap_callback);
  ros::Subscriber gridmap_inflate_sub = nh.subscribe<quadrotor_msgs::OccMap3dOld>("gridmap_inflate", 1, gridmap_inflate_callback);
