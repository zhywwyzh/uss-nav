...
#include <fake_traj_opt/lbfgs_raw.hpp>
...
static inline double objectiveFunc(void* ptrObj,
  ...
  double sumT;
  if(landing_){
    sumT = obj.sum_T_ + deltaT * deltaT;
  }else{
    sumT = obj.sum_T_ + deltaT * deltaT;
  }
  DEBUG_MSG( " --------------------------------------------- ");
  DEBUG_MSG("deltaT^2: " << deltaT * deltaT);
  forwardT(t, sumT, T);
  ...
  # ifdef DEBUG
  Trajectory traj = obj.jerkOpt_.getTraj();
  obj.visPtr_->visualize_traj(traj, "traj");

  std::vector<Eigen::Vector3d> wpt_on_traj;
  std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>> rays;
  double t_traj = 0;
  for (int i = 0; i < (int)obj.tracking_ps_.size(); ++i){
    wpt_on_traj.push_back(traj.getPos(t_traj));
    rays.emplace_back(obj.tracking_ps_[i], traj.getPos(t_traj));
    t_traj += obj.tracking_dt_;
  }
  obj.visPtr_->visualize_pointcloud(wpt_on_traj, "/wpt_on_traj");
  obj.visPtr_->visualize_pairline(rays, "/track_rays");

  int a;
  if(obj.is_debug)  std::cin >> a;
  // usleep(100*1e3);
  #endif

  double cost = obj.jerkOpt_.getTrajJerkCost();
  DEBUG_MSG("jerk cost: " << cost);
...
TrajOpt::TrajOpt(ros::NodeHandle& nh) : nh_(nh) {
  ...
  nh.getParam("rhoTracking_t", rhoTracking_t_);
  nh.getParam("rhoV_t", rhoV_t_);
  nh.getParam("rhoA_t", rhoA_t_);
  nh.getParam("rhoTracking_l", rhoTracking_l_);
  nh.getParam("rhoV_l", rhoV_l_);
  nh.getParam("rhoA_l", rhoA_l_);
  ...
  nh.getParam("tolerance_d_t", tolerance_d_t_);
  nh.getParam("tolerance_d_l", tolerance_d_l_);

  nh.getParam("rhoLanding_v", rhoLanding_v_);
  nh.getParam("landing_a_xy_limit", landing_a_xy_limit_);
  nh.getParam("landing_a_z_limit", landing_a_z_limit_);
...
bool TrajOpt::generate_traj(const Eigen::MatrixXd& iniState,
  ...
  rhoV_ = rhoV_t_;
  rhoA_ = rhoA_t_;
  rhoTracking_ = rhoTracking_t_; 
  tolerance_d_ = tolerance_d_t_;
...
bool TrajOpt::generate_traj(const Eigen::MatrixXd& iniState,
  ...
  rhoV_ = rhoV_l_;
  rhoA_ = rhoA_l_;
  rhoTracking_ = rhoTracking_l_; 
  tolerance_d_ = tolerance_d_l_;
  cfgHs_ = hPolys;
  if (cfgHs_.size() == 1) {
    cfgHs_.push_back(cfgHs_[0]);
  }
...
void TrajOpt::addTimeIntPenalty(double& cost) {
  ...
  std::vector<Eigen::Vector4d> traj_wpts;
  ...
  for (int i = 0; i < N_; ++i) {
    ...
    for (int j = 0; j < innerLoop; ++j) {
      ...
      if(s1<1e-5){
        traj_wpts.emplace_back(pos.x(), pos.y(), pos.z(), i);
      }
      ...
      if (grad_cost_p_corridor(pos, hPoly, grad_tmp, cost_tmp)) {
        ...
        corridor_cost += omg * step * cost_tmp;
      }
  ...
  #ifdef DEBUG
  visPtr_->visualize_pointcloud_intensity(traj_wpts, "/debug_traj_wpts");
  #endif
  DEBUG_MSG("Corridor cost: " << corridor_cost);
  DEBUG_MSG("V cost: " << v_cost);
  DEBUG_MSG("A cost: " << a_cost);
...
void TrajOpt::addTimeCost(double& cost) {
  ...
  Eigen::Vector3d grad_tmp;
  double cost_tmp;
  Eigen::Matrix<double, 6, 3> gradViolaPc, gradViolaVc;

  double track_dis_cost = 0.0;
  double track_angle_cost = 0.0;
  double landing_p_cost = 0.0;
  double landing_v_cost = 0.0;
  ...
  for (int i = 0; i < M; ++i) {
    ...
    if (landing_) {
      if (grad_cost_p_landing(pos, target_p, grad_tmp, cost_tmp)) {
        ...
        landing_p_cost += rho * step * cost_tmp;
        jerkOpt_.gdC.block<6, 3>(piece * 6, 0) += rho * step * gradViolaPc;
        if (piece > 0) {
          jerkOpt_.gdT.head(piece).array() += -rho * step * grad_tmp.dot(vel);
        }
      }
      if (grad_cost_v_landing(pos, vel, target_p, target_v_, grad_tmp, cost_tmp)) {
        gradViolaVc = beta1 * grad_tmp.transpose();
        cost += rho * step * cost_tmp;
        landing_v_cost += rho * step * cost_tmp;
        jerkOpt_.gdC.block<6, 3>(piece * 6, 0) += rho * step * gradViolaVc;
        if (piece > 0) {
          jerkOpt_.gdT.head(piece).array() += -rho * step * grad_tmp.dot(acc);
        }
      }
    } else {
      if (grad_cost_p_tracking(pos, target_p, grad_tmp, cost_tmp)) {
        gradViolaPc = beta0 * grad_tmp.transpose();
        cost += rho * step * cost_tmp;
        track_dis_cost += rho * step * cost_tmp;
        jerkOpt_.gdC.block<6, 3>(piece * 6, 0) += rho * step * gradViolaPc;
        if (piece > 0) {
          jerkOpt_.gdT.head(piece).array() += -rho * step * grad_tmp.dot(vel);
        }
      ...
      if (grad_cost_visibility(pos, target_p, tracking_visible_ps_[i], tracking_thetas_[i],
        ...
        gradViolaPc = beta0 * grad_tmp.transpose();
        cost += rho * step * cost_tmp;
        track_angle_cost += rho * step * cost_tmp;
        jerkOpt_.gdC.block<6, 3>(piece * 6, 0) += rho * step * gradViolaPc;
        if (piece > 0) {
          jerkOpt_.gdT.head(piece).array() += -rho * step * grad_tmp.dot(vel);
        }
  ...
  if (landing_){
    DEBUG_MSG("landing_p_cost: " << landing_p_cost);
    DEBUG_MSG("landing_v_cost: " << landing_v_cost);
  }else{
    DEBUG_MSG("track_dis_cost: " << track_dis_cost);
    DEBUG_MSG("track_angle_cost: " << track_angle_cost);
  }
}
...
bool TrajOpt::grad_cost_p_corridor(const Eigen::Vector3d& p,
  ...
  gradp.setZero();
  costp = 0;
  double clearance_d = clearance_d_;
  if(landing_) clearance_d = 0.0;
  for (int i = 0; i < hPoly.cols(); ++i) {
    Eigen::Vector3d norm_vec = hPoly.col(i).head<3>();
    double pen = norm_vec.dot(p - hPoly.col(i).tail<3>() + clearance_d * norm_vec);
    if (pen > 0) {
      double pen2 = pen * pen;
...
bool TrajOpt::grad_cost_v_landing(const Eigen::Vector3d& p, const Eigen::Vector3d& v,
                                  const Eigen::Vector3d& target_p, const Eigen::Vector3d& target_v,
                                  Eigen::Vector3d& gradv,
                                  double& costv) {
  Eigen::Vector3d dp = (p - target_p);
  Eigen::Vector3d dv = (v - target_v);

  bool ret;
  gradv.setZero();
  costv = 0;

  // DEBUG_MSG("pos: "<<p.transpose());
  // DEBUG_MSG("dv:  "<<dv.transpose());
  // DEBUG_MSG("dv:  "<<dv.norm());
  // DEBUG_MSG("dvmax: "<<dv_max);
  // DEBUG_MSG(" ");

  double dv_max = sqrt(2*landing_a_xy_limit_*dp.head(2).norm());
  double vpen = dv.head(2).squaredNorm() - dv_max*dv_max;
  if(vpen > 0){
    gradv.head(2) += 6 * vpen * vpen * dv.head(2);
    costv += vpen * vpen * vpen;
    ret = true;
  }

  dv_max = sqrt(2*landing_a_z_limit_*dp.z());
  vpen = dv.z()*dv.z() - dv_max*dv_max;
  if(vpen > 0){
    gradv.z() += 6 * vpen * vpen * dv.z();
    costv += vpen * vpen * vpen;
    ret = true;
  }
...
bool TrajOpt::grad_cost_visibility(const Eigen::Vector3d& p,
  ...
  Eigen::Vector3d b(cos(track_angle_expect_), sin(track_angle_expect_), 0);
  ...
  double cosTheta = 1.0;
  // std::cout<<"angle: "<<acos(inner_product / norm_a / norm_b)*180/M_PI<<std::endl;
