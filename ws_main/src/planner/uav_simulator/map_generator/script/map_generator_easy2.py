...
class map_generator(object):
    ...
    def init_params(self):
        ...
        self.dimx = rospy.get_param("map/x_size", default=30.0)
        self.dimy = rospy.get_param("map/y_size", default=4.0)
        ...
        self.all_map_topic = rospy.get_param(
            "all_map_topic", default="/map_generator/global_cloud")
        self.all_map_pub = rospy.Publisher(
            self.all_map_topic, PointCloud2, queue_size=1)
    ...
    def add_box(self, size, position):
        ...
        x_low = math.floor(
            (position[0] - size[0] / 2) / self.resolution) * self.resolution + 0.5 * self.resolution
        x_high = math.floor(
            (position[0] + size[0] / 2) / self.resolution) * self.resolution + 0.5 * self.resolution
        y_low = math.floor(
            (position[1] - size[1] / 2) / self.resolution) * self.resolution + 0.5 * self.resolution
        y_high = math.floor(
            (position[1] + size[1] / 2) / self.resolution) * self.resolution + 0.5 * self.resolution
        z_low = math.floor(
            (position[2] - size[2] / 2) / self.resolution) * self.resolution + 0.5 * self.resolution
        z_high = math.floor(
            (position[2] + size[2] / 2) / self.resolution) * self.resolution + 0.5 * self.resolution
    ...
    def add_cylinder(self, size, position):
        ...
        center_y = position[1]
        z_low = math.floor(
            (position[2] - size[1] / 2) / self.resolution) * self.resolution + 0.5 * self.resolution
        z_high = math.floor(
            (position[2] + size[1] / 2) / self.resolution) * self.resolution + 0.5 * self.resolution
                ...
                if radius2 < (radius_num + 0.5) ** 2:
                    ...
                    while z <= z_high:
                        if radius2 > (radius_num - 0.5) ** 2 or \
                            ...
                            self.points.append(
                                [center_x + x * self.resolution, center_y + y * self.resolution, z])
                        z += self.resolution
                y += 1
            x += 1
    ...
    def add_chair(self, position):
        ...
        if direction == 0:
            ...
            self.add_box_on_floor(
                [x, 0.1, h * h_ratio], [position[0], position[1] + x / 2, h * h_ratio / 2])
        elif direction == 2:
            self.add_box_on_floor(
                [x, 0.1, h * h_ratio], [position[0], position[1] - x / 2, h * h_ratio / 2])
        elif direction == 3:
            self.add_box_on_floor(
                [0.1, x, h * h_ratio], [position[0] + x / 2, position[1], h * h_ratio / 2])
        elif direction == 4:
            self.add_box_on_floor(
                [0.1, x, h * h_ratio], [position[0] - x / 2, position[1], h * h_ratio / 2])
    ...
    def add_bed(self, position):
        ...
        if np.random.rand() > 0.5:
            self.add_box_on_floor(
                [x, y, z], [-self.dimx / 2 + x / 2, position[1], z/2])
            if np.random.rand() > 0.3:
                self.add_box_on_floor(
                    [0.1, y, z*1.5], [-self.dimx / 2 + 0.1, position[1], z*1.5/2])
        else:
            self.add_box_on_floor(
                [x, y, z], [self.dimx / 2 - x / 2, position[1], z/2])
            if np.random.rand() > 0.3:
    ...
    def add_table(self, position):
        ...
        # lower part
        if np.random.rand() > 0.5:
            self.add_cylinder_on_floor(
                [x/2 * ratio, z], [position[0] * 0.6, position[1]])

        else:
            self.add_box_on_floor(
                [x * ratio, y * ratio, z], [position[0] * 0.6, position[1]])
    ...
    def add_long_door(self, position):
        ...
        # left side
        self.add_box_on_floor([pos_x - x / 2 + self.dimx / 2, y, self.dimz],
                              [-self.dimx / 2 + (pos_x - x / 2 + self.dimx / 2) / 2, pos_y, self.dimz / 2])
        # right side
        self.add_box_on_floor([self.dimx / 2 - pos_x - x / 2, y, self.dimz], [
                              self.dimx / 2 - (self.dimx / 2 - pos_x - x / 2) / 2, pos_y, self.dimz / 2])
    ...
    def make_map(self):
        ...
        # floor
        self.add_layer([50, 3.0*self.dimy], [0, 0, 0.1])
        # ceiling
        # self.add_layer([self.dimx, self.dimy], [0,0, self.dimz - self.floor_bias])
        # boundary
        # left
        self.add_box([0.10, self.dimy, self.dimz],
                     [-self.dimx / 2, 0, self.dimz / 2.0])

        # right
        self.add_box([0.10, self.dimy, self.dimz], [
                     self.dimx / 2, 0, self.dimz / 2.0])
        # top
        self.add_box([8.0, 0.10, self.dimz], [
                     4.0, self.dimy / 2, self.dimz / 2.0])

        self.add_box([5.0, 0.10, self.dimz], [
                     17.5, self.dimy / 2, self.dimz / 2.0])

        self.add_box([self.dimx / 2, 0.10, self.dimz],
                     [-self.dimx / 4, self.dimy / 2, self.dimz / 2.0])
        ...
        # room 1
        self.add_box([0.10, 4.0, self.dimz], [
                     4.0, self.dimy/2.0+2.0, self.dimz / 2.0])
        self.add_box([0.10, 4.0, self.dimz], [
                     4.0+8.0, self.dimy/2.0+2.0, self.dimz / 2.0])

        self.add_box([8.0, 0.1, self.dimz], [
                     4.0+4.0, self.dimy/2.0+4.0, self.dimz / 2.0])

        # room 2
        self.add_box([0.10, 4.0, self.dimz], [
                     20.0, self.dimy/2.0+2.0, self.dimz / 2.0])
        # self.add_box([0.10, 4.0, self.dimz], [4.0+8.0, self.dimy/2.0+2.0, self.dimz / 2.0])

        self.add_box([8.0, 0.1, self.dimz], [
                     16.0, self.dimy/2.0+4.0, self.dimz / 2.0])
       ...
       # wall 1
        self.add_box([0.10, self.dimy/3.0, self.dimz], [
                     -self.dimx / 4, self.dimy/4.0+0.3, self.dimz / 2.0])
       # wall 2
        self.add_box([0.10, self.dimy/3.0, self.dimz], [
                     self.dimx / 4, -self.dimy/4.0-0.3, self.dimz / 2.0])

       # wall 3
        self.add_box([0.10, self.dimy, 1.0], [
                     0.0, 0.0, 1.0])
        ...
        # table 1
        self.add_box([2.0, 0.8, self.dimz],
                     [-4, -self.dimy/2.0+0.4, self.dimz / 2.0])
        # table 2
        self.add_box([2.0, 0.8, self.dimz], [
                     4, self.dimy/2.0-0.4, self.dimz / 2.0])
        # table 2
        # self.add_cylinder([0.8, 1.6], [-3, -0.2, 0.4])
        ...
        # bed 1
        # self.add_box([2.7, 2.0, 1.5], [5.5, 5, 0.25])
        # # nightstand 1
        ...
        # self.add_box([2.5, 0.3, 3.0], [-0.45, 1.8, 1.5])
        # self.add_box([0.3, 4.0, 3.0], [0, 2.9, 1.5])
