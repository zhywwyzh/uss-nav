...
void addWalls(const Eigen::Vector3d& position, const Eigen::Vector3d& size, pcl::PointCloud<pcl::PointXYZ>& cloud)
{
  double x_low  = floor((position[0] - size[0] / 2) / _resolution) * _resolution + 0.5 * _resolution;
  double x_high = floor((position[0] + size[0] / 2) / _resolution) * _resolution + 0.5 * _resolution;
  double y_low  = floor((position[1] - size[1] / 2) / _resolution) * _resolution + 0.5 * _resolution;
  double y_high = floor((position[1] + size[1] / 2) / _resolution) * _resolution + 0.5 * _resolution;
  double z_low  = floor((position[2] - size[2] / 2) / _resolution) * _resolution + 0.5 * _resolution;
  double z_high = floor((position[2] + size[2] / 2) / _resolution) * _resolution + 0.5 * _resolution;
  ...
  while (x <= x_high)
  {
    ...
    while (y <= y_high)
    {
      ...
      while (z <= z_high)
      {
        if ((abs(x - x_low) < _resolution) || (abs(x - x_high) < _resolution) ||
            (abs(y - y_low) < _resolution) or (abs(y - y_high) < _resolution) ||
            (abs(z - z_low) < _resolution) or (abs(z - z_high) < _resolution))
            {
...
void RandomMapGenerate()
{
  ...
  for (int i = 0; i < _obs_num; i++)
  {
    ...
    for (double xd = -w; xd <= w; xd += _resolution)
      for (double yd = -w; yd <= w; yd += _resolution)
      {
        h = rand_h(eng);
        pt_random.x = xd + x;
        pt_random.y = yd + y;
        for (double zd = -1.0; zd < h; zd += _resolution)
        {
          pt_random.z = zd;
          cloudMap.points.push_back(pt_random);
        }
...
void RandomMapGenerateCylinder()
{
  ...
  for (int i = 0; i < _obs_num && ros::ok(); i++)
  {
    ...
    double theta_y = rand_theta_(eng) / 2;
    double theta_p = rand_theta_(eng) / 2;
    double theta_r = rand_theta_(eng) / 2;
    Eigen::Vector3d eulerAngle(theta_y, theta_p, theta_r);
    ...
    for (int r = -widNum / 2.0; r < widNum / 2.0; r++)
      for (int s = -widNum / 2.0; s < widNum / 2.0; s++)
      {
        ...
        {
          double temp_x = (r + 0.5) * _resolution + 1e-2;
          double temp_y = (s + 0.5) * _resolution + 1e-2;
          double temp_z = (t + 0.5) * _resolution + 1e-2;
          Eigen::Vector3d temp_p(temp_x, temp_y, temp_z);
          temp_p = rotate * temp_p;
          temp_p += Eigen::Vector3d(x, y, 0);
  ...
  for (int i = 0; i < circle_num_; ++i)
  {
    ...
    double theta_y = rand_theta_(eng);
    double theta_p = rand_theta_(eng);
    double theta_r = rand_theta_(eng);
    Eigen::Vector3d eulerAngle(theta_y, theta_p, theta_r);
    Eigen::AngleAxisd rollAngle(Eigen::AngleAxisd(eulerAngle(0), Eigen::Vector3d::UnitX()));
    Eigen::AngleAxisd pitchAngle(Eigen::AngleAxisd(eulerAngle(1), Eigen::Vector3d::UnitY()));
    Eigen::AngleAxisd yawAngle(Eigen::AngleAxisd(eulerAngle(2), Eigen::Vector3d::UnitZ()));
  ...
  // walls
  const double EXPAND = 80.0;
  for (double y = _y_l - EXPAND; y <= _y_h + EXPAND; y += 0.1)
    for (double z = -0.5; z <= 3.0; z += 0.1)
    {
      pt_random.x = _x_l - EXPAND;
      pt_random.y = y;
      pt_random.z = z;
      cloudMap.push_back(pt_random);
    }
  for (double y = _y_l - EXPAND; y <= _y_h + EXPAND; y += 0.1)
    for (double z = -0.5; z <= 3.0; z += 0.1)
    {
      pt_random.x = _x_h + EXPAND;
      pt_random.y = y;
      pt_random.z = z;
      cloudMap.push_back(pt_random);
    }
  for (double x = _x_l - EXPAND; x <= _x_h + EXPAND; x += 0.1)
    for (double z = -0.5; z <= 3.0; z += 0.1)
    {
      pt_random.x = x;
      pt_random.y = _y_l - EXPAND;
      pt_random.z = z;
      cloudMap.push_back(pt_random);
    }
  for (double x = _x_l - EXPAND; x <= _x_h + EXPAND; x += 0.1)
    ...
    {
      pt_random.x = x;
      pt_random.y = _y_h + EXPAND;
      pt_random.z = z;
      cloudMap.push_back(pt_random);
    }
  // for (double x = _x_l - 10; x <= _x_h + 10; x += 0.1)
  //   for (double y = _y_l - 10; y <= _y_h + 10; y += 0.1)
  //   {
  //     pt_random.x = x;
  //     pt_random.y = y;
  //     pt_random.z = 0.0;
...
void RandomMapGenerateBugtrap()
{
  ...
  double x_low  = -2.0 * step_size;
  double x_high =  2.0 * step_size;
  double y_low  = -1.0 * step_size + y_offset;
  double y_high =  2.0 * step_size + y_offset;
  ...
  for (int i = 0; i < _obs_num && ros::ok(); i++)
  {
    ...
    if (x < x_low || x > x_high || y < y_low || y > y_high) continue;
    if ( sqrt((_init_x - x) * (_init_x - x) + (_init_y - y) * (_init_y - y)) <  clear_r) continue;
  ...
  // walls
  addWalls(Eigen::Vector3d(-1.0 * step_size,  2.0 * step_size + y_offset, 0.0), Eigen::Vector3d(2 * step_size, 0.2, z_h), cloudMap);
  addWalls(Eigen::Vector3d( 1.5 * step_size,  2.0 * step_size + y_offset, 0.0), Eigen::Vector3d(1 * step_size, 0.2, z_h), cloudMap);
  addWalls(Eigen::Vector3d(-0.5 * step_size,  1.0 * step_size + y_offset, 0.0), Eigen::Vector3d(1 * step_size, 0.2, z_h), cloudMap);
  addWalls(Eigen::Vector3d( 0.0 * step_size,  0.0 * step_size + y_offset, 0.0), Eigen::Vector3d(2 * step_size, 0.2, z_h), cloudMap);
  addWalls(Eigen::Vector3d( 0.0 * step_size, -1.0 * step_size + y_offset, 0.0), Eigen::Vector3d(4 * step_size, 0.2, z_h), cloudMap);

  addWalls(Eigen::Vector3d(-2.0 * step_size,  0.5 * step_size + y_offset, 0.0), Eigen::Vector3d(0.2, 3 * step_size, z_h), cloudMap);
  addWalls(Eigen::Vector3d(-1.0 * step_size,  0.5 * step_size + y_offset, 0.0), Eigen::Vector3d(0.2, 1 * step_size, z_h), cloudMap);
  addWalls(Eigen::Vector3d( 0.0 * step_size,  1.5 * step_size + y_offset, 0.0), Eigen::Vector3d(0.2, 1 * step_size, z_h), cloudMap);
  addWalls(Eigen::Vector3d( 1.0 * step_size,  1.0 * step_size + y_offset, 0.0), Eigen::Vector3d(0.2, 2 * step_size, z_h), cloudMap);
  addWalls(Eigen::Vector3d( 2.0 * step_size,  0.5 * step_size + y_offset, 0.0), Eigen::Vector3d(0.2, 3 * step_size, z_h), cloudMap);
...
void RandomMapGenerateManual()
{
  ...
  for (double x = -10.5; x <= -4.5; x += 0.1)
    for (double y = -0.5; y <= 0.5; y += 0.1)
      for (double z = -1.0; z <= 2.5; z += 0.1)
      {
        pt_random.x = x;
        pt_random.y = y;
        pt_random.z = z;
...
int main(int argc, char **argv)
{
  ...
  // unsigned int seed = 1811754953;
  cout << "seed=" << seed << endl;
  eng.seed(seed);

  // RandomMapGenerate();
  // RandomMapGenerateManual();
  // RandomInclinedColumn();

  // RandomMapGenerateCylinder();

  RandomMapGenerateBugtrap();
